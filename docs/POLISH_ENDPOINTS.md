...
`models` 的 key 必須是伺服器接受的 model ID，可以包含 `/`。`polish.model` 第一個 `/` 前為 provider ID，其餘為完整 model ID。不要使用內建保留的 `apple/foundation`、`builtin/rules` 作為 endpoint 模型選擇值。

App 的 **Smart Polish Model** 選單可暫時覆寫模型，直到重新啟動或選擇 **Use Configuration File**。要永久指定模型請修改 JSON。使用 **Reload Polish Configuration** 更新選單；每次開始 Smart Polish 錄音也會重新載入設定，錄音途中修改檔案不會改變該次使用的 endpoint。
